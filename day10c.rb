...
def solve(arg)
  ...
  max_x = maze[0].size - 1
  locations = []
  locations << [start_x - 1, start_y, :west] if start_x - 1 >= 0 && %w[- L F].include?(maze[start_y][start_x - 1])
  locations << [start_x + 1, start_y, :east] if start_x + 1 <= max_x && %w[- J 7].include?(maze[start_y][start_x + 1])
  locations << [start_x, start_y - 1, :north] if start_y - 1 >= 0 && %w[| 7 F].include?(maze[start_y - 1][start_x])
