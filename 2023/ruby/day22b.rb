...
class Brick
  ...
  attr_reader :y_range
  attr_reader :z_range
  attr_reader :supporting_bricks
  attr_reader :bricks_on_top
  attr_reader :id
  ...
  def initialize(x_range, y_range, z_range, id)
    ...
    @y_range = y_range
    @z_range = z_range
    @supporting_bricks = Set.new
    @bricks_on_top = Set.new
    @id = id
  end
  ...
  def total_above_fall(set, set2)
    return if set2.include?(self)
    # puts "set: #{set.map(&:id)}, set2: #{set2.map(&:id)}, #{self.inspect}"

    bricks_on_top.each do |b|
      # puts "set: #{set.map(&:id)}, b: #{b.inspect}"
      set << b if b.supporting_bricks.subset?(set)
    end
    set2 << self
    bricks_on_top.each do |b|
      b.total_above_fall(set, set2) if set.include?(b)
    end
    # puts "set@end: #{set.map(&:id)}\n\n"
    set.size - 1
  end
...
def solve(arg)
  ...
  bricks.each do |brick|
    if !brick.can_destroy?
      numb = brick.total_above_fall(Set.new([brick]), Set.new)
      # puts "brick: #{brick.inspect}: #{numb}"
      count += numb
    end
  end
...
def calc_gravity(bricks, max_x, max_y)
  cross_section = Array.new(max_y + 1) { Array.new(max_x + 1, nil) }
...
def parse_input(input)
  ...
  end.map.with_index do |((x1, y1, z1), (x2, y2, z2)), i|
