...
class Year2023::Day23h < Base
  ...
  def calc_longest_path(graph)
    paths = [] of Int16
    queue = [{0_u8, graph.to_a.sort_by { |from, _| from }.map { |_| false }, 0_i16}]
    while !queue.empty?
      node, seen, steps = queue.shift
      # puts "\nnode: #{node}, steps: #{steps}, seen: #{seen}}"
      if node == 1
        paths << steps
        next
      end
      ...
      graph[node].each do |new_node, additional_steps|
        # puts "new_node: #{new_node}, include?: #{seen[new_node]}"
        next if seen[new_node]
        if need_clone
          seen = seen.clone
        else
          need_clone = true
