...
class Year2023::Day20c < Base
  def solve(arg)
    ...
    lcm = periods.reduce(1_u64) do |acc, period|
      acc.lcm(period)
    end
  ...
  def calc_number(modules, start_mod_id)
    ...
    while mod_id
      ...
      numb += 1 << i if !conjunction_dests.includes?(mod_id)
  ...
  def parse_input(input)
    modules = {} of String => Tuple(Symbol, String, Array(String))
    input.split('\n').each do |str|
      mod = :repeater
      if str[0] == '%'
        mod = :flip_flop
