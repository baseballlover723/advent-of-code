...
class Year2023::Day22b < Base
  class Brick
    ...
    def total_above_fall(set, set2)
      return 0 if set2.includes?(self)
      # puts "set: #{set.map(&:id)}, set2: #{set2.map(&:id)}, #{self.inspect}"

      bricks_on_top.each do |b|
        # puts "set: #{set.map(&:id)}, b: #{b.inspect}"
        set << b if b.supporting_bricks.subset_of?(set)
      end
      set2 << self
      bricks_on_top.each do |b|
        b.total_above_fall(set, set2) if set.includes?(b)
      end
  ...
  def solve(arg)
    ...
    bricks.each do |brick|
      if !brick.can_destroy?
        numb = brick.total_above_fall(Set{brick}, Set(Brick).new)
