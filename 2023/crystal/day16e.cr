...
# parallelized
# Not actually in parallel because it slows down the other scripts
class Year2023::Day16e < Base
  def solve(arg)
  ...
  def max_mirror_energy(input)
    ...
    outputs = Channel(Int32).new
    traversals.map do |(r, c, d)|
      spawn do
        outputs.send(input.clone.traverse(r, c, d))
      end
    end.map do |_|
      outputs.receive
    end.max
