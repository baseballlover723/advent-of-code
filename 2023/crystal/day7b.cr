...
class Year2023::Day7b < Base
  ...
  def calc_rank(cards)
    count_hash = cards.tally
    jokers = count_hash[1]? || 0
    if jokers > 0
      # puts "count_hash: #{count_hash}"
      return {6, :five_of_a_kind} if jokers >= 5
      count_hash.delete(1)
    end

    count = count_hash.to_a.sort_by { |card, count| -count }
    count[0] = {count[0][0], count[0][1] + jokers}
    # puts "count: #{count}"
    case count.size
    when 5
      {0, :high_card}
