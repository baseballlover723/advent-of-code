...
class Year2023::Day3a < Base
    ...
    str_matrix.each.with_index do |row, y|
      ...
      row.each_char.with_index do |c, x|
        ...
        parts.reject! do |(numb, x_range, y_range)|
          next false if y < y_range.begin
          next false if !x_range.includes?(x)
  ...
  def parse_input(input)
    parts = [] of Tuple(Int32, Range(Int32, Int32), Range(Int32, Int32))
    ...
    split_input.each.with_index do |str, y|
      ...
      str.each_char.with_index do |c, i|
        if c.ord - ZERO >= 0 && c.ord - ZERO < 10
          ...
          if !current_numb.empty? && !start.nil?
            s = {start - 1, 0}.max
            e = {i, str.size - 1}.min
            ys = {y - 1, 0}.max
